...
**Current state (2026-07-24):** **M0 ✅**, **M1 ✅**, **M2 ✅**, **M3 ✅**, **M4 ✅**, **M5 ✅**, **M6 🔶 (connection + remote exec, OpenSSH-validated)**, **M7 🔶 (SFTP v3 client+server, loopback)**. Core/Client/Server
...
| **M6** | 🔶 | Connection layer: **channels + window-based flow control, `exec`/`shell` + `pty-req`/`env` accept, `exit-status`, capture stdout/stderr done ✅** (`SshConnection.ExecuteAsync` client + `ServeExecAsync` server + `SshExecContext`/`SshCommandResult`/`SshExecHandler`) — **the "log in, run a command, capture output, log out" core, OpenSSH-validated** (real `ssh user@host cmd` runs on our server, gets our output + exit code; loopback incl. 200 KiB multi-packet). ⬜ still: `SshCommand` streaming/stdin API, subsystem dispatch (→ M7 SFTP), `SSH_MSG_PING`/`PONG`, keepalive/dead-peer + idle timeouts, session recording (asciicast) | remote-exec E2E ✅ (real `ssh` exec vs our server, exit-status 0 and 42); loopback both roles ✅ | L |
| **M7** | 🔶 | **SFTP v3 client + server done ✅** (`SftpClient` upload/download/list/stat/mkdir/remove/rename over a channel duplex; `SftpServer` dispatch to `ISftpFileSystem`; `InMemorySftpFileSystem`; `SftpProtocol`/`SftpFileAttributes`/`SftpStatusCode`; subsystem channels `SshConnection.Open/AcceptSubsystemAsync` + `SshChannelDuplex` flow control) — loopback upload/download (100 KiB multi-chunk) + list/rename/remove + error status green. ⬜ still: `LocalSftpFileSystem` (root jail), **access profiles** (`SshAccessProfile`/`SftpPermissions` upload-only/download-only), quotas & bandwidth, real `sftp`/`psftp` interop, extensions/pipelining, `SftpFileStream` | loopback client↔server ✅; real `sftp` CLI, profiles, quotas pending | L |
