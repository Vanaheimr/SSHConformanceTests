...
**Current state (2026-07-24):** **M0 ✅**, **M1 ✅**, **M2 ✅**, **M3 ✅**, **M4 ✅**, **M5 ✅**, **M6 🔶 (connection + remote exec + streaming `SshCommand` + keepalive/idle liveness, OpenSSH-validated)**, **M7 🔶 (SFTP v3 client+server, loopback)**. Core/Client/Server
...
| **M6** | 🔶 | Connection layer: **channels + window-based flow control, `exec`/`shell` + `pty-req`/`env` accept, `exit-status`, capture stdout/stderr done ✅** (`SshConnection.ExecuteAsync` client + `ServeExecAsync` server + `SshExecContext`/`SshCommandResult`/`SshExecHandler`) — **the "log in, run a command, capture output, log out" core, OpenSSH-validated** (real `ssh user@host cmd` runs on our server, gets our output + exit code; loopback incl. 200 KiB multi-packet). **Streaming `SshCommand` done ✅** (`SshConnection.StartCommandAsync` → `SshCommandProcess`: incremental stdout/stderr `Stream`s, piped stdin with channel flow control, `env`/`pty-req`, `WaitForExitAsync`; concurrent `ServeCommandAsync` server that consumes stdin as it streams). **Keepalive/idle timeouts done ✅** (`SshLivenessMonitor` state machine — `ClientAlive*`/`ServerAlive*` equiv — driven by `TimeProvider`; `SshConnectionOptions` wires `keepalive@openssh.com` probing + dead-peer + idle-timeout into `SshCommandProcess`; `SSH_MSG_PING`/`PONG` handled). Deterministic liveness unit tests (N probes, *not one short*; idle only on true inactivity; responsive-but-silent peer still times out) + loopback streaming/keepalive integration. ⬜ still: session recording (asciicast v2), subsystem dispatch beyond SFTP, `SshCommandLine.Quote` | remote-exec E2E ✅; streaming stdout/stdin + large-output loopback ✅; liveness deterministic + idle-timeout/keepalive integration ✅ | L |
| **M7** | 🔶 | **SFTP v3 client + server done ✅** (`SftpClient` upload/download/list/stat/mkdir/remove/rename over a channel duplex; `SftpServer` dispatch to `ISftpFileSystem`; `InMemorySftpFileSystem`; `SftpProtocol`/`SftpFileAttributes`/`SftpStatusCode`; subsystem channels `SshConnection.Open/AcceptSubsystemAsync` + `SshChannelDuplex` flow control) — loopback upload/download (100 KiB multi-chunk) + list/rename/remove + error status green; **access profiles ✅** (`SshAccessProfile.SftpUploadOnly`/`SftpDownloadOnly` + `SftpPermissions` gate the server — log-upload / firmware-download presets). ⬜ still: `LocalSftpFileSystem` (root jail), quotas & bandwidth, real `sftp`/`psftp` interop, extensions/pipelining, `SftpFileStream` | loopback client↔server ✅; real `sftp` CLI, profiles, quotas pending | L |
