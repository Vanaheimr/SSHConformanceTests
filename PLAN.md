...
| **M10** | 🔶 | **Demo CLI ✅** (`hermod-ssh`: `keygen`/`scan`/`ca`/`exec`/`serve`/**`connect`**/**`sftp`**/**`forward`** working end-to-end — all driven through the `SshClient`/`SshServer` façade; `serve` offers exec + `--sftp-root` SFTP + loopback forwarding at once; verified against a live server: exec+connect stream, sftp put/ls/get round-trips byte-for-byte, forward `-L` binds+tunnels). **README updated ✅** (feature status + CLI examples). XML docs land per file as written. ⬜ still: `play` verb (asciicast replay), NuGet packaging, BenchmarkDotNet baseline | demo keygen→scan→ca pipeline test ✅; exec/connect/sftp/forward + serve E2E via façade ✅ | S–M |
