...
- **Nightly** 🔶 (`.github/workflows/nightly.yml`, since 2026-08-13), split into a fatal and an informational half the way Hermod's own nightly is — a red that can mean "a mirror was slow" must not share a colour with a red that means our code broke:
  ...
  - *informational:* the **forward half of the version spread** — the newest upstream OpenSSH, discovered from the release listing rather than pinned, built from source and prepended to `PATH` for the OpenSSH fixtures (which resolve their binaries through `PATH` already, so this needs no harness change). It is the only way to reach the behaviour gates named in §11.1, because no distribution ships them: trixie stops at 10.0p2 while upstream is at 10.5p1 (verified 2026-08-13). **First run, 10.5p1:** every check driven by their `ssh` client against our server passed, `mlkem768x25519-sha256` and `sntrup761x25519-sha512` included — that is the forward evidence this half exists to produce, and it is already in hand. The eleven that failed all needed *their* `sshd` and were our build's fault, not the release's: `--without-pam` makes sshd reject the `-o UsePAM=no` the fixture sends unconditionally, and an uninstalled tree has neither a config path nor the `sshd-session` helper that 9.8+ re-executes per connection. Fixed by installing into a private prefix under `RUNNER_TEMP`
