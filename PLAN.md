...
⬜ still: Go `x/crypto/ssh` and curl+libssh — the WSL harness now covers process-driven peers in both roles; CI still needs the decision in §13.5 | audit catalog tests (envelope/sequence/overflow) ✅; keystroke-timing decorrelation deterministic ✅; nightly matrix job = CI todo | M–L |
