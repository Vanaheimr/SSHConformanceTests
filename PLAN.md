...
**Per-commit CI landed ✅ (2026-08-13):** [.github/workflows/ci.yml](.github/workflows/ci.yml) — GitHub Actions, the same two legs as Hermod's own CI (`windows-latest` + `debian:13` container, `fail-fast: false`), recursive submodule checkout, plus `openssh-client` and a job-scoped ssh-agent in the container. Measured on a local Debian 13 before committing: **41 of 94 checks run, 53 skip, 0 fail** — against **7 of 94** with the same tree and no provisioning. The Debian leg is the one that proves the post-quantum work: OpenSSH 10.0p2 on OpenSSL 3.5.6 completes **both** hybrid key exchanges against our server (`mlkem768x25519-sha256`, `sntrup761x25519-sha512`). **Confirmed on the runners at the first attempt** (master `754afad`, 2026-08-13): both legs green — the Debian container in 70 s (apt 8 s, recursive checkout 3 s, build 34 s, test 7 s) and Windows in 157 s (build 55 s, test 34 s), with the two Linux-only steps correctly skipped on the Windows leg. That settles the three things local verification structurally could not reach: the apt-before-checkout bootstrap, the recursive submodule clone over the public HTTPS URLs, and the Windows leg at all. Two things it did not settle have since been settled as well. **Fixed (`24b7f8e`):** the per-leg pass/skip counts used to vanish with the console log when a run aged out; the test step now also writes TRX and each leg uploads it as an artifact (`test-results-windows-latest` / `test-results-debian-13-container`, retained 90 days), which `InteropReport` consumes directly — so the interop matrix can finally be generated from CI runs rather than only from local ones. Downloading an artifact needs authentication, so an anonymous observer still sees only the verdict. For scale, the same suite on a developer Windows box with WSL present runs **70 of 94** rather than the container's 41, because there the eight WSL-gated fixtures actually execute. **Answered (2026-08-13):** the `pull_request` path. That run had failed having recorded **zero jobs**, i.e. before any job started, six seconds after which the merge landed and took `refs/pull/1/merge` (and later the run's own record) with it — a workflow defect would have failed *inside* a job, so the suspicion was that the merge overtook its own PR run. Three Dependabot pull requests have since run the path properly, each recording both jobs and going green on both legs, which confirms it: the path works, and that one run was a merge race rather than a defect. The concurrency block has since been changed so a push can no longer produce the same class of hole in the other direction — see the commit that keys pushes on the SHA. **The harness blocker is gone ✅ (2026-08-13):** `interop/WslInterop.cs` used to refuse on anything but Windows, which grounded all eight remaining fixtures (AsyncSSH, curl\libssh2, Dropbear, Go `x/crypto/ssh`, OpenSSH-as-server, Paramiko, plink, TinySSH) on Linux regardless of what was installed. It now runs the same commands natively, with the platform difference confined to three places — command dispatch, path translation and host resolution. Measured on Debian 13 with the peers provisioned: **93 of 94 run, 1 skips, 0 fail, in 13 s** (the skip is the ssh-agent check, which CI's agent step already covers, so a provisioned runner sees 94/94). Windows is unchanged at 91/94, verified across three runs. **The nightly landed too ✅ (2026-08-13):** [.github/workflows/nightly.yml](.github/workflows/nightly.yml), split into a fatal half (the pinned suite three times, for the flakes a single run per commit cannot distinguish from noise) and an informational one (the newest upstream OpenSSH, built from source) — the shape and the first two nights' results are in §11.6. The provisioning-and-cost question this line used to pose answered itself when `setup-wsl.sh` went into the per-commit gate: the peers cost seconds, so the gate takes them and the nightly buys the *version spread* instead. 🔶 still: only the backward half of that spread — older OpenSSH releases through era-matched container images. | audit catalog tests (envelope/sequence/overflow) ✅; keystroke-timing decorrelation deterministic ✅; nightly job ✅, its matrix half forward-only (§11.6) | M–L |
