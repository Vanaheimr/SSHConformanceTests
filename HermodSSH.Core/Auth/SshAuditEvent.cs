...
namespace org.GraphDefined.Vanaheimr.Hermod.SSH
{
    ...
    /// <summary>A pre-authentication banner was sent to the peer (SSH_MSG_USERAUTH_BANNER).</summary>
    public sealed record BannerSentEvent(DateTimeOffset Timestamp) : SshAuditEvent(Timestamp);

    /// <summary>A single authentication method succeeded for the user (may be one factor of several).</summary>
    ...
    /// <summary>A single authentication attempt failed. The reason is for the audit log only, never the wire.</summary>
    ...
    /// <summary>Authentication completed successfully (all required factors satisfied).</summary>
    ...
    /// <summary>Authentication failed and the connection is being torn down (attempt budget exhausted).</summary>
    ...
    /// <summary>A sink for the typed audit event stream.</summary>
    public interface ISshAuditSink
    {
        /// <summary>Record one audit event.</summary>
        ValueTask WriteAsync(SshAuditEvent Event, CancellationToken CancellationToken = default);
    }


    /// <summary>A sink that invokes a callback for each event (e.g. to bridge to a logging framework).</summary>
    public sealed class DelegateAuditSink : ISshAuditSink
    {
        ...
        /// <summary>Create a delegate-backed sink.</summary>
        public DelegateAuditSink(Func<SshAuditEvent, CancellationToken, ValueTask> Handler)
        {
            this.handler = Handler;
        }

        /// <summary>Create a delegate-backed sink from a synchronous callback.</summary>
    ...
    /// <summary>An in-memory sink that collects events, mainly for tests and diagnostics.</summary>
    public sealed class CollectingAuditSink : ISshAuditSink
    {

        private readonly ConcurrentQueue<SshAuditEvent> events = new ();

        /// <summary>All events recorded so far, in order.</summary>
